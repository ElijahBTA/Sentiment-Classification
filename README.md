...
1. How do you classify a new tweet? Give a short description of the main steps.
    ...
    > I have a for loop that runs until i is bigger then the size of the tweet which then breaks out of the for loop meaning the tweet is done. Then it continues in my while loop until end of file repeating the same process.

2. What is the time complexity of your training implementation (Big-Oh notation)? Explain why.

   > My time complextiy is O(N) since my for loop is complete by the size of the tweet. My for loop is not using like <4 it is using words.size() which is O(N).

3. What is the time complexity of your classification implementation (Big-Oh notation)? Explain why.

   > Should be O(N) since i am looping through each sentiment value of testing and training to compare them.

4. How do you know that you use proper memory management? That is that you do not have
   a memory leak?

   > When using the new keyword i make sure that there is a delte[]. Also the use of valgrind.

5. What was the most challenging part of the assignment?

   > For some reason the most challening part of the project was increasing my percentage and understanding what the project consited of.
